...
const allTestPlans = readdirSync(testDataPath).filter(path => path.endsWith(".testplan"))
...
describe('all testplans run as expected', () => {
    ...
    test.skip.each(allTestPlans.filter(p => skippedTests.includes(p)))('testplan: %p', async (testplan: string) => { });

    // Test all other testplans
    test.each(allTestPlans.filter(p => !skippedTests.includes(p)))('testplan: %p', async (testplan: string) => {
        const testPlanPath = resolve(testDataPath, testplan);
        const testPlan = TestPlan.fromFile(testPlanPath);

        const compiledYarnFilePath = testPlanPath.replace(/.testplan/, ".yarnc");
        const linesFilePath = testPlanPath.replace(/.testplan/, "-Lines.csv");
        const metadataFilePath = testPlanPath.replace(/.testplan/, "-Metadata.csv");

        let program = Program.fromBinary(readFileSync(compiledYarnFilePath));
        ...
        for (const run of testPlan.runs) {
            ...
            const continueTestPlan = () => {
                    ...
                    try {
                        ...
                        if (currentStep instanceof ExpectLineStep) {
                            ...
                            vm.lineCallback = async (line) => {
                                expect(line).toEqual(currentStep.expectedText);
                                if (currentStep.expectedHashTags.length > 0) {
                                    console.warn("Not testing hashtags at the moment")
                                }
                                continueTestPlan();
                            }
                            return;
                        ...
                        if (currentStep instanceof ExpectCommandStep) {
                            ...
                            vm.commandCallback = async (command) => {
                                expect(command).toEqual(currentStep.expectedText);
                                continueTestPlan();
                            }
                            return;
                        }
                        if (currentStep instanceof ActionSelectStep) {
                            ...
                            vm.optionCallback = async (options) => {
                                ...
                                for (let i = 0; i < options.length; i++) {
                                    ...
                                    expect(option.line).toEqual(expectedOption.line);
                                    expect(option.lineCondition).toEqual(expectedOption.available);
                                    if (expectedOption.hashtags.length > 0) {
                                        console.warn("hashtag checking not implemented");
                                    }
                                }
                                expectedOptions = []
                                continueTestPlan();

                                // test plans select one greater than the options
                                return currentStep.selectedIndex;
                            }
                            return;
                        }
                        if (currentStep instanceof ActionSetVariableStep) {
                            vm.variableStorage[currentStep.variableName] = currentStep.value;
                        }
                ...
                // We've reached the end of the testplan. Expect a stop and no other content.
                vm.optionCallback = dontExpectOptions;
                vm.commandCallback = dontExpectCommand;
                vm.lineCallback = dontExpectLines;
                vm.dialogueCompleteCallback = async () => {};
